<?php
// Insert family
if (isset ( $jsonObj ["spInf1"] ) && isset ( $jsonObj ["spInf2"] ) && isset ( $jsonObj ["spInf3"] ) && isset ( $jsonObj ["spInf4"] ) && isset ( $jsonObj ["spInf5"] ) && isset ( $jsonObj ["idToRemove"] ) && isset ( $jsonObj ["idToSet"] )) {
	if (! empty ( $jsonObj ["spInf1"] ) && ! empty ( $jsonObj ["spInf2"] ) && ! empty ( $jsonObj ["spInf3"] ) && ! empty ( $jsonObj ["spInf4"] ) && ! empty ( $jsonObj ["spInf5"] )) {
		...
		if (! $database) {
			...
			$query = "INSERT INTO sp_fam (spInf1, spInf2, spInf3, spInf4, spInf5) VALUES (";
