    ...
    if (isset($_GET["mid"])) {
        if (!empty($_GET["mid"])) {
            ...
            if (!$database) {
                ...
                $query_mem = "SELECT spInf2";
                $query_mem .= " FROM sp_terrmaps";
                $query_mem .= " WHERE spInf3 =";
...
<body class="bg-dark">
    ...
    if (!isset($language) || empty($territoryID) || empty($mapsID)) {
        ...
        <table class="table table-striped table-dark">
            <thead>
                <tr>
                    <th scope="col" class="text-center"><?php echo $language['mapsviewererror'] ?></th>
                </tr>
            </thead>
            <tbody>
            </tbody>
