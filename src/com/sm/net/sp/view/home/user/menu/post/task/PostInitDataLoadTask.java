...
public class PostInitDataLoadTask implements TaskInterface {
	...
	public PostInitDataLoadTask(AlertBuilder alertBuilder, Settings settings, Stage stage, Post view) {
		super();
	...
	public void start(HashMap<String, Object> hashMap) {
		...
		JSONObject jsonPDFDest = JSONRequest.PDFDEST_LOAD();
		JSONObject jsonPDFReplace = JSONRequest.PDFREPLACE_LOAD();

		JSONObject jsonPost = JSONRequest.POST_LOAD();

		post(hashMap, url, jsonPDFDest, "responsePDFDest");
		post(hashMap, url, jsonPDFReplace, "responsePDFReplace");
		post(hashMap, url, jsonPost, "responsePost");
...
//		post(hashMap, url, jsonSongs, "responseSongs");
	...
	public void feedback(HashMap<String, Object> hashMap) {
		...
		ObservableList<PDFDest> pdfDestList = pdfDestListFeedback(hashMap);
		ObservableList<PDFReplace> pdfReplaceList = pdfReplaceListFeedback(hashMap);
		ObservableList<PostNews> postNewsList = postNewsListFeedback(hashMap);

		this.view.setPdfDestList(pdfDestList);
		this.view.setPdfReplaceList(pdfReplaceList);
...
//		this.view.checkConfigs();
	...
	private ObservableList<PostNews> postNewsListFeedback(HashMap<String, Object> hashMap) {
		...
		JSONObject response = (JSONObject) hashMap.get("responsePost");
		if (response != null) {
	...
	private ObservableList<PDFReplace> pdfReplaceListFeedback(HashMap<String, Object> hashMap) {

		ObservableList<PDFReplace> pdfReplaceList = FXCollections.observableArrayList();

		JSONObject response = (JSONObject) hashMap.get("responsePDFReplace");
		if (response != null) {
			...
			if (status == 0) {
				...
				for (Object obj : result) {
					...
					pdfReplaceList.add(new PDFReplace(spInfID, spInf1, spInf2, spInf3));
				}

				pdfReplaceList.sort((s1, s2) -> s1.getSpInfID().compareTo(s2.getSpInfID()));
	...
	private ObservableList<PDFDest> pdfDestListFeedback(HashMap<String, Object> hashMap) {

		ObservableList<PDFDest> pdfDestList = FXCollections.observableArrayList();

		JSONObject response = (JSONObject) hashMap.get("responsePDFDest");
		if (response != null) {
